...
sudo nala install $BROWSER $CLI $CONTAINERIZATI $CPP $DOCS $EDITORS $GREETER $ICONS $IMAGE $JAVA $JAVASCRIPT $PAINT $PYTHON $READER $SCM $SHELLS $TERMINAL $THEMES_CURSOR $THEMES_GTK $TUI $VIDEO $WALLPAPERS $WINDOW_MANAGER
